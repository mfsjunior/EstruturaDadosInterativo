class AVLModule extends BaseModule {
    ...
    init() {
        ...
        this.initDebugEngine('avl');
        
        this.tree = new AVLTree();
        this.operationPanel = new AVLOperationPanel(this);
        this._bindPlaybackControls();

        globals.codeHighlighter.setFileName('AVLTree.java');
        globals.statePanel.updateProp('head', '-');
        globals.statePanel.updateProp('tail', 'size@0');
        globals.statePanel.updateProp('size', 0);
        globals.consolePanel.log('AVL inicializada. Insercoes aplicam rotacoes para manter a arvore balanceada.');
    ...
    runScenario(scenarioId) {
        ...
        this.scenarioManualMode = false;
        this.pendingScenarioLabel = scenario.label || 'Cenario AVL';
    ...
    _bindPlaybackControls() {
        ...
        this._handleRestart = () => {
            ...
            this.resetSystem();
            const globals = this.appManager.getGlobals();
            globals.consolePanel.log('AVL reinicializada.');
            document.getElementById('btnPlayPause').textContent = String.fromCodePoint(0x25B6);
        };
