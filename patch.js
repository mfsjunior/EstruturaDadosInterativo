...
const path = 'c:/projetos/EstruturaDeDados/ED Interativo/src/model/DoublyLinkedList.js';
let content = fs.readFileSync(path, 'utf8');

const startStr = '    addFirst(value) {';
const endStr = '    // Mtodos para facilitar o state interno visual (Modo Professor)';
let startIdx = content.indexOf(startStr);
let endIdx = content.indexOf(endStr);

if (endIdx === -1) {
    endIdx = content.indexOf('    getState() {');
}

if (startIdx !== -1 && endIdx !== -1) {
    const newMethods =     addFirst(value) {
...
;
    ...
    const newContent = content.substring(0, startIdx) + newMethods + content.substring(endIdx);
