...
with open('c:/projetos/EstruturaDeDados/ED Interativo/src/model/DoublyLinkedList.js', 'r', encoding='utf-8') as f:
    content = f.read()

# We want to replace everything from ddFirst(value) { to the end of _unlink(node) { ... } which is before getState().
start_idx = content.find('    addFirst(value) {')
end_idx = content.find('    // Mtodos para facilitar o state interno visual (Modo Professor)')
if end_idx == -1:
    end_idx = content.find('    getState() {')

if start_idx != -1 and end_idx != -1:
    new_methods = '''    addFirst(value) {
        ...
        this._addStep('CREATE_NODE', { node: node, activeLine: 2 }, code, Criando novo Node() na memoria: );

        node.next = this.head;
        this._addStep('SET_NEXT', { source: node.id, target: this.head ? this.head.id : null, activeLine: 3 }, code, 
ode.next aponta para a cabeca atual ().);
        ...
        this.size++;
        this._addStep('UPDATE_SIZE', { size: this.size, activeLine: 7 }, code, Tamanho atualizado para .);
    ...
    addLast(value) {
        ...
        this._addStep('SET_PREV', { source: node.id, target: this.tail ? this.tail.id : null, activeLine: 3 }, code, 
ode.previous aponta para o tail atual.);
        ...
        this.tail = node;
        this._addStep('SET_TAIL', { target: node.id, activeLine: 6 }, code, 	ail agora aponta para o novo Node.);
    ...
    removeFirst() {
        ...
        this._addStep('INFO', { value: value, activeLine: 3 }, code, Salvando o valor da cabeca ().);
        ...
        this._addStep('ISOLATE_NODE', { target: removedNodeId, activeLine: 7 }, code, O no contendo  ficou isolado e sera limpo pelo Garbage Collector.);

        this.size--;
        this._addStep('UPDATE_SIZE', { size: this.size, activeLine: 7 }, code, Tamanho reduzido para .);
    ...
    get(index) {
        ...
        if (index < 0 || index >= this.size) {
            this._addStep('ERROR', { msg: 'IndexOutOfBoundsException', activeLine: 2 }, code, Erro: indice  fora dos limites (0 ate ).);
            return null;
        }
        ...
        for (let i = 0; i < index; i++) {
            node = node.next;
            this._addStep('TRAVERSE_STEP', { target: node.id, i: i+1, activeLine: 5 }, code, Caminhando engate por engate (i=).);
        }

        this._addStep('TRAVERSE_END', { target: node.id, value: node.value, activeLine: 7 }, code, Chegou ao indice . Valor = .);
        this._addStep('COMPLEXITY', { value: 'O(n)', desc: Foram percorridos  nos ate achar a posicao. A memoria nao e sequencial., activeLine: 8 }, code, 'Operacao get concluida.');
    ...
    removeValue(value) {
        ...
        let node = this.head;
        let count = 0;
        this._addStep('TRAVERSE_START', { target: node ? node.id : null, activeLine: 2 }, code, Iniciando busca do valor ...);

        while (node !== null) {
            count++;
            this._addStep('TRAVERSE_COMPARE', { target: node.id, value: node.value, activeLine: 4 }, code, Comparando:  == );
            
            if (node.value == value) {
                this._addStep('INFO', { activeLine: 5 }, code, Valor  encontrado no no .);
                this._unlink(node);
                
                this._addStep('COMPLEXITY', { value: 'O(n)', desc: Busca:  iteracoes (O(n)). Unlink isolado: O(1). Dominante: O(n)., activeLine: 6 }, code, 'Operacao removeValue concluida.');
                return true;
            }
            node = node.next;
            if(node !== null) {
        ...
        this._addStep('INFO', { activeLine: 10 }, code, Valor  nao existe na lista.);
    ...
    _unlink(node) {
        ...
        this._addStep('UPDATE_SIZE', { size: this.size, activeLine: 6 }, code, Tamanho reduzido para .);
    }

'''
    
    new_content = content[:start_idx] + new_methods + content[end_idx:]
    with open('c:/projetos/EstruturaDeDados/ED Interativo/src/model/DoublyLinkedList.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
